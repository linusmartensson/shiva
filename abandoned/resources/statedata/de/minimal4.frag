...
void main(void)
{
	...
	float v = max(dot(pow(abs(u),vec2(16.0)),vec2(1.0))*100.0,0.0)*clamp(60-time,0.0,1.0);
	c = vec4(vec3(v),1.0)*vec4(2.7,1.4,0.8,1.0)+vec4((sin(glp.xyz*0.001)),0.0)*20.0+20.0;
	c.rgb += max(s.x*100.0,0.0);
}
