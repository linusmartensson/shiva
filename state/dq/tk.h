...
struct tgdata{
	...
	tgdata() : reload(false), m(new music("statedata/dq/Linus.mp3")), set("resources/statedata/dq/set.act"), clon("resources/statedata/dq/clon.act"),
...
class tk : public shiva::behaviour {
	...
	virtual void render(shiva::childvec children, boost::any &data) const {
		...
		d.fb.render([&]{
			...
			if(!d.m->pause() && d.clon.curr != 0){
				...
				d.clp.run("engine", d.frames[d.set.curr].count);
			}
	...
	virtual void init(shiva::state *instance, boost::any &data) const {
		...
		d.c.init(instance, "statedata/dq/0.camera");
		d.refresh();
		for(int i=0;i<5;++i){
			pcl::PCDReader pr;
			std::ostringstream oss;

			pcl::PointCloud<pcl::PointXYZRGB>::Ptr ifcloud(new pcl::PointCloud<pcl::PointXYZRGB>);

			oss<<"resources/mergedclouds-real/"<<i<<".merge";
			Log::info()<<"loading "<<oss.str()<<std::endl;
			if(pr.read(oss.str(), *ifcloud) == 0){
				
				d.frames.push_back(framedata());
				d.frames.back().b = new point[ifcloud->size()];
				d.frames.back().count = ifcloud->size();
				size_t q = 0;
				auto b = d.frames.back().b;
				for(auto j = ifcloud->begin(); j!=ifcloud->end();++j,++q){
					b[q].x = j->x*1000.f;
					b[q].y = j->y*1000.f;
					b[q].z = j->z*1000.f;
					b[q].r = j->r;
