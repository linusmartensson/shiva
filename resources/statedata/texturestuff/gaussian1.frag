...
void main(void)					
{
	...
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)+ivec2(0,5), 0) * weight[5];
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)-ivec2(0,5), 0) * weight[5];
	
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)+ivec2(0,6), 0) * weight[6];
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)-ivec2(0,6), 0) * weight[6];
	
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)+ivec2(0,7), 0) * weight[7];
	color += texelFetch(tex,ivec2(gl_FragCoord.xy)-ivec2(0,7), 0) * weight[7];
	...
	if(gl_FragCoord.y+7 >= res.y || gl_FragCoord.y-7 < 0) color.rgba = vec4(0.0);
}
