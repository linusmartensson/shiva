...
float dist(vec3 r){
	...
	float z = 
		max(sdPlane(r+vec3(0.0,r.x-500.0,0.0), vec4(0.0,1.0,0.0,0.0)),
			sdPlane(r-vec3(0.0,r.x+500.0,0.0), vec4(0.0,1.0,0.0,0.0)));
	z = max(z, sdBox(r-vec3(0.0,-500.0,2500.0), vec3(900.0,1500.0,2000.0)));
	return z;
}
vec3 norm(vec3 r){
	vec2 e = vec2(1.0,0.0);
...
void main(void)
{
	...
	if(d<lim){
		...
		c.rgb = vec3(dot(n,normalize(vec3(1.0,1.0,-1.0))));
	} else {
		 c = vec4(-0.3,0.1,0.1,0.1);
		 a=vec4(0.0,0.0,0.0,sin(dot(ray,ray)*320.0)*4.0+7.0);
	}
	a.w = sin(dot(uv.y+10.0,(1.0+uv.x)*12134.0))*7.0+9.0;
	c*=10.0;
}
