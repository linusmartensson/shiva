...
void main(void)
{
	...
	if(abs(d) < lim && d >= 0.0){
		a.xyz = norm(ray)*5000.0;
		a.w=abs(sin(w.z*3200.0+w.x*13200.0+w.y*35200.0))*20.0+0.5;
		
		vec4 cc = (ray*camera);
		c = 
			vec4(10.0+sin(w.x)*4.0,5.0,5.0-sin(w.x)*4.0,1.0)-a.y;
		c = clamp(c,vec4(0.0),vec4(100.0));
