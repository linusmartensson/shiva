...
float dist(vec3 r){
	float u = 0.0;//snoise(r.xyz*0.001)*250.0;
	u+=snoise(r.xyz*0.001)*1550.0;
	r.y += 250;
	
	float z = sdPlane(r, vec4(0.0,1.0,0.0,0.0))+u;
	z = max(z, 500.0-length(r.xy));
	return z;
}
vec3 norm(vec3 r){
	vec2 e = vec2(1.0,0.0);
...
void main(void)
{
	...
	for(step=1.0;step < dp;){
		k+=1.0;
		d = dist(ray+dir*step*dm);
		lim = 1.0+abs(100.0*step/dp)+k*0.01;
		if(d < lim) {
			break;
		}
		step += d;
	...
	if(d<lim){
		...
		vec3 n = norm(ray+dir*step*dm);

		a.xyz = n*sin(n)*7000.0;
		a.xyz = a.xzy;
		
		c = vec4(0.5,sin(dot(w,w))*1.0+1.6,5.3,1.0)*dot(a,a)*0.000000005-(-2.0-w.y*0.1)*0.01;
		c = c.gbra;
		a.w = sin(dot(step*dir,step*dir))*1.0+2.0;
		c.a = 0.0;
		//uniform mat4 camera;
		vec4 v = w*camera;
		gl_FragDepth = v.z/v.w;

	} else {
		 c = vec4(-0.3,0.1,0.1,0.1);
		 a=vec4(0.0,0.0,0.0,sin(dot(ray,ray))+2.0);
		 gl_FragDepth = 1.0;
	}
