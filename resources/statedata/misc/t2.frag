...
float dista(vec3 r, float fa, float fk){
	...
	cs.z = -cs.y;
	r.z -= 15000.0;
	r.xz = vec2(cs.x*r.x+cs.z*r.z,cs.y*r.x+cs.x*r.z);

	z = snoise(r.xyz*0.00015)/0.0004;


	z = min(max(-z,length(r)-10000.0),length(r)-9900.0);

	z = min(z, sdBox(r+vec3(0.0,25000.0,0.0), vec3(1000.0,10000.0,1000.0)));
	z = min(z, sdBox(r-vec3(0.0,25000.0,0.0), vec3(1000.0,10000.0,1000.0)));
	z = min(z, sdBox(r+vec3(25000.0,0.0,0.0), vec3(10000.0,1000.0,1000.0)));
	z = min(z, sdBox(r-vec3(25000.0,0.0,0.0), vec3(10000.0,1000.0,1000.0)));
	z = min(z, sdBox(r+vec3(25000.0,0.0,0.0).yyx, vec3(10000.0,1000.0,1000.0).yyx));
	z = min(z, sdBox(r-vec3(25000.0,0.0,0.0).yyx, vec3(10000.0,1000.0,1000.0).yyx));
	return z;
}
...
vec4 cmod(vec3 ray, float fa, float fk, vec2 aa, vec2 ca, vec2 da, vec2 ga, vec3 dir, float dm, float k, float tf, float dp, float step){
	...
	texOut.xyz = mix(vec3(0.0,0.4,1.0),vec3(0.0,1.0,0.0),smoothstep(9900.0,10000.0,distance(or, vec3(0.0,0.0,15000.0))));
	
	texOut.xyz = mix(texOut.xyz, vec3(1.0,1.0,1.0), smoothstep(10000,100000,distance(or,vec3(0.0,0.0,15000.0))));
	
	
	texOut.xyz *= ld;
	texOut.xyz += vec3(1.0,0.1,0.1)*clamp(ld,0.0,1.0)*ld*ld*ld*ld;
	
	
	
	texOut.xyz = mix(texOut.xyz, vec3(0.0), smoothstep(100000,200000,distance(or,vec3(0.0,0.0,15000.0))));
	texOut.a=1.0;
